...
void printCoords(listNodePtr currentPtr, int *max) {
   ...
   int j, a;           
	while (currentPtr != NULL) {
      ...
      /**
       * When printing out the moves, because moves under 10 print out as 1-9 and moves over have two digits as X1-X9
       * The output format was weird until I solved it by adding the following:
       * 
      If moves are less than 10 */
      if (i < 10)
         printf("Move 0%d: (%c %d) | ", i, a + 1 - 1 + 'A', j + 1);
      //If moves are more than 10
      else
