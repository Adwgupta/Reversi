   ...
   for (int j = 0; j < BOARD_SIZE; j++) {
      ...
      for (int i = 0; i < BOARD_SIZE; i++) {
         ...
         if (board[j][i].type == BLACK)
            printf("   B   |"); //This will print out a black background text to serve as the black marker

         //For white discs
         if (board[j][i].type == WHITE)
            printf("   W   |"); //This will print out a black background text to serve as the black marker
